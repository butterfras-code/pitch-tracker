...
export class SessionView {
 ...
 <div class="session-workspace"><aside id="sessionSidebar" class="session-sidebar" aria-label="Session controls">${action('session-sidebar', 'Hide controls', 'id="hideControls" aria-expanded="true" aria-controls="sessionSidebar"')}<div class="row view-picker" role="group" aria-label="Content view">${['split', 'student', 'class'].map((v) => action('session-view', v[0].toUpperCase() + v.slice(1), `data-view="${v}" aria-label="${v[0].toUpperCase() + v.slice(1)} view"`)).join('')}${action('session-fullscreen', 'Full screen')}</div> <div class="session-bar" aria-label="Session controls"><div class="listening-control">${action('pause-listening', 'Pause listening', 'id="pauseListening" class="primary"')}<small id="classroomStatus" role="status"></small></div>${action('undo', 'Undo last change', 'id="sessionUndo"')}${action('session-settings', 'Settings', 'aria-label="Session behavior settings" aria-expanded="false" aria-controls="behaviorSettings" popovertarget="behaviorSettings"')}${action('end-session', 'Finish session')}</div>
