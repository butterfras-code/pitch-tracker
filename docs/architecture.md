...
Successful manual and microphone recordings both call the application's `PitchFeedback` presenter after recording and auto-advance. The popup uses the recorded student's name, a factual rating, a directional arrow/checkmark and a shuffled phrase. It is a native modal dialog in the static shell, so it works in fullscreen without changing split-view geometry. Continue or Escape dismiss modal feedback and restore native focus; inline card feedback retains its Continue action. The configured duration is a minimum when the microphone is listening and another turn is possible: automatic dismissal also requires an observed quiet or settled-background handoff. Audio analysis and microphone telemetry continue behind feedback, but scoring and clap navigation stay disabled. A pause heard during feedback arms the next turn without requiring another pause after dismissal. Manual dismissal never supplies pause evidence. With the microphone off, listening paused, or the round complete, feedback keeps its ordinary timed dismissal. Session keyboard shortcuts remain blocked while feedback is visible. Live pitch changes do not trigger popups.
...
Enabling the microphone starts automatic scoring. Auto Advance applies to both manual and microphone results: Until correct retries incorrect results; One and done advances any completed attempt. Navigation follows the current round order, stops at round completion, and never deletes attempts when going back. The toolbar Shuffle action randomizes the temporary round order without changing the saved roster or recording an attempt. Undo covers shuffling and navigation as well as scores. Round completion is temporary view state; restarting or revisiting a student allows another turn.
...
Student history is always available from a compact card-header action. Roster cards do not render attempt counts or scoring controls; the single live tuner owns manual scoring in Split, Student, and Class views. All controls continue to use the named delegated-event registry.
...
Retry navigation and shuffling stay within the retry queue. Nonmembers are labeled Not in this round rather than untested. Deliberately selecting or manually scoring a nonmember adds that student to the temporary round; Undo restores the previous membership.
...
`PitchHold` integrates elapsed milliseconds, not frame counts, over a bounded recent evidence window. Short interruptions extend that window and its minimum elapsed time by at most 15% of the configured hold duration, capped at 300 ms. The progress indicator pauses on incompatible readings; missing audio receives no credit. It requires a full observation window before saving, and credits only intervals bracketed by reliable observations of the same status within the configured pitch spread. Within each status, it finds the most-supported frequency band no wider than that spread. Correct results require 85% of the window in that band (1,700 ms in a 2,000 ms window). To keep the extension from accepting wandering pitch, the winning band must also contain at least 85% of all credited evidence in the extended window. Low/high results retain the stricter requirement of a full hold duration of stable evidence, all within one band. The saved frequency is the time-weighted median of supporting measurements, evaluated against the current target and A4. It cannot average low and high into correct or combine different octaves for an exact-octave target. The current observation must agree with both the previous reliable observation and the winning band; a result is never completed during silence or an unresolved octave jump.

A missing-pitch interruption longer than 350 ms, a callback gap over 250 ms, or explicit cancellation clears evidence. Old evidence also ages out of the rolling window, so isolated short successes cannot accumulate indefinitely. After at least 160 ms of compatible acquisition, readings near half or double the established frequency (within the configured spread, capped at 50 cents) are treated as interruptions. At least three consistent readings spanning 250 ms confirm a real octave change and start a fresh hold. The initial octave is always evaluated as measured, and no target-based octave snapping or fabricated frequency enters a saved score. The bounded extension compensates for the two reliable endpoints needed to resume credit. Short hold settings necessarily have less interruption tolerance at the approximately 80 ms analysis cadence. The 85% rule is an internal policy; the existing hold and stability settings and v1 saved-data format are preserved. A settings change cancels evidence before applying the new target/tuning.
...
A4 tuning, hold duration, allowed spread, and noise gate use native sliders with visible values, units, endpoint explanations, and keyboard support. Detection changes save automatically. Instrument pitch changes remain explicit drafts; Save cancels active pitch evidence before applying them. Session/history labels and CSV exports include custom offsets. Offline Chromium and Firefox tests exercise the built HTML, note/clef changes, dragging and keyboard input, reload persistence, and actual custom-tone playback and measurement through the test audio-device boundary.
...
`src/session-layout.css` owns session geometry and is loaded after theme paint. A shared toolbar holds Back to classes, the icon-based Split/Student/Class selectors, the fullscreen toggle, Class navigation, the combined Advance selector, clap navigation, a split microphone control, Shuffle, Undo, and Finish session. The microphone control's main area starts, pauses, or resumes listening in sync with the control beside the target; its separate dropdown opens device selection and the explicit microphone-off action without changing listening. Unboxed Previous/Next controls stay with the current student in every view, including the active card in Class view. The site header remains above the session; native fullscreen hides it until exit.

Clap navigation is a toolbar toggle with a persistent accessible description. Enabling it shows a nonmodal two-line instruction toast for 1.5 seconds while retaining button focus. The selected control indicates when listening is required. Session notes and active-student notes are historical data only and have no live-session actions. Feedback popup duration remains in Settings. Reopening a saved session reports contextually that round progress restarts while saved attempts remain available.
...
The full-screen Split workspace normally allocates two-thirds of its width to the current student and one-third to student cards; tall desktop windows stack the target and tuner and give more width to the roster. Student uses the full content area. Class removes the separate student panel and places the same target/playback and tuner/scoring DOM in the active card. Moving these components preserves their element identity and the audio detector’s current hold; views never record an attempt or restart listening. Manual scoring remains available without a working microphone. The round footer remains accessible in all views.
...
Live session microphone level stays above the tuner, followed by manual range scoring. Listening status appears with the target controls. The student display retains current/upcoming identities; Class cards independently indicate Current/Up next and the latest recorded rating. Audio detection publishes transient `data-range` on the session shell using the existing pitch evaluator; silence, pause and student changes clear it. This presentation state is never persisted. Themes define `range-animation` (default glow; Big Button wiggle; Pitch Press sparkle). Shared CSS provides an active outline and suppresses animation for reduced-motion preferences.
...
Session metadata lives in the toolbar outside both content panes. The roster owns one scroll container, including a separate active card when search/filter excludes the current student. Card updates are patched in place and the live practice components are temporarily returned to the student panel before patching, then placed in the active card. Class cards reserve practice space to avoid a new turn rearranging the grid. Class feedback uses the existing feedback timer and detection gate but renders inside the scored card, with Continue and automatic dismissal. The latest rating remains after feedback; an active student can then try again. Split/Student retain the full feedback dialog. No saved-data format or scoring rules change.
...
Classes and Settings have separate main-navigation entries. Settings occupies the remaining viewport below the shared header without document scrolling. A fixed left rail selects Instruments, Detection, or Defaults on landscape displays; narrow displays use the same controls as a horizontal strip. Each section owns its overflow while its heading and actions remain visible.

## Settings workspace and persistence

Instruments renders one selected instrument at a time. The selector and Add/Delete controls live in the fixed section header; note and octave controls share a row with the local SVG staff on landscape displays, and the accepted-range editor spans below them. Add creates an unsaved draft. Save commits only the displayed instrument while preserving every other configuration; Revert reloads persisted data. Switching instruments, settings sections, or top-level navigation warns before discarding a dirty pitch draft. Deletion is refused while any class student references the instrument. Staff SVGs always use white backgrounds with black notation across themes.

Detection settings, feedback timing, and session defaults save automatically. Detection and Defaults therefore have no competing form-level save buttons. Failed default or pitch saves restore the previous in-memory data and report the failure.

Explicitly saving Session defaults or feedback timing upgrades validated data to schema 3. The top-level `sessionDefaults` object stores Auto Advance, advance mode, clap navigation, and starting content view (`auto` preserves Student on phones and Split otherwise); its retired `teacher` field remains validation-compatible but is ignored by the UI. `settings.feedbackDurationMs` stores a nullable popup duration. Either upgrade path initializes the complete schema-3 shape. The migration clones its input, upgrades v1 pitch targets through v2, preserves historical measurements, and seeds defaults from the legacy Auto Advance value with other original initial values. Saving pitch settings never downgrades v3. Older versions remain readable and are not upgraded by merely opening Settings. The existing storage key remains unchanged. New backups require a version-3-capable release.

Saving defaults leaves current session controls unchanged. Defaults apply on creation, resume, class switching, app initialization, and backup restoration. Session overrides do not rewrite the defaults; advance continues using the existing `settings.advance` working value, while mode/claps and view remain runtime state. Reopening reapplies the saved defaults. Microphone activation is manual, device choice remains local runtime state, and class selection, playback, notes, navigation, pause, and fullscreen remain session actions. Reinitializing the workspace disposes its old event listeners. Failed default saves restore the previous in-memory data and show an error.

Unit tests cover v1/v2 migration, v3 backup/storage round trips, preservation through pitch saves, and invalid defaults rejected before any restore writes. Offline browser coverage exercises section navigation, draft warnings and reverts, one-at-a-time instrument editing, protected deletion, automatic persistence, default/current-session isolation, actual auto-advance and clap navigation, and save failures.

### Compact session cards and stable telemetry

Roster cards place instrument and a compact saved-rating badge above the name; an active student's saved result is prefixed Last to separate it from current pitch evidence. Compact history and attendance controls share the card's top-right corner. History is always available, attempts remain saved without displaying a tries count, and manual scoring lives only in the active tuner in both Split and Class views.

Class cards reserve 340px at ordinary desktop scales and share a compact target/microphone row, tuner and manual scoring. The target surface always pairs control/control-ink theme tokens, including material themes such as Vintage Audio. The microphone level uses an accessible meter and a fixed icon with on/off/paused/waiting states; the waiting state uses the overridable yellow `mic-waiting` theme token and an accessible “Microphone waiting for a pause” label; it has no changing status text. Live pitch and signed cents use a fixed-height readout with a dash for unavailable evidence. Guidance, microphone errors, and scoring remain distinct from meter telemetry. Geometry tests exercise silence, noisy input, pitch changes, pause, saved ratings and turn changes under offline file URLs. No saved data or scoring rules change.
