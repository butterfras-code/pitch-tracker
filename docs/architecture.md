...
`src/session-layout.css` owns session geometry and is loaded after theme paint. A shared toolbar holds Back to classes, Split/Student/Class selection, fullscreen, Session options, Undo, metadata, and Finish session. Session options is a native light-dismissible popover for behavior and microphone configuration. It uses theme tokens, anchored placement with a bounded fallback, and keyboard dismissal. Unboxed Previous/Next chevrons sit immediately beside the current student name in every view; listening stays beside target playback. The site header remains above the session; native fullscreen hides it until exit.
