...
`src/session-layout.css` owns session geometry and is loaded after theme paint. A shared toolbar holds Back to classes, View, Session options, Undo, metadata, and Finish session. View is a native light-dismissible popover containing a segmented Split/Student/Class selector, fullscreen, and Teacher details. Session options is a separate popover for behavior and microphone configuration. Both use theme tokens, anchored placement with a bounded fallback, and keyboard dismissal. Unboxed Previous/Next chevrons sit immediately beside the current student name in every view; listening stays beside target playback. The site header remains above the session; native fullscreen hides it until exit.
...
The full-screen Split workspace normally allocates two-thirds of its width to the current student and one-third to student cards; tall desktop windows stack the target and tuner and give more width to the roster. Student uses the full content area. Class removes the separate student panel and places the same target/playback and tuner/scoring DOM in the active card. Moving these components preserves their element identity and the audio detector’s current hold; views never record an attempt or restart listening. Manual scoring remains available without Teacher details or a working microphone. The round footer remains accessible in all views.
...
Roster cards place instrument and a compact saved-rating badge above the name; an active student's saved result is prefixed Last to separate it from current pitch evidence. The direct attendance icon sits beside the name, removing the dedicated attendance footer. Narrow Split cards wrap turn position below the name instead of squeezing the name between controls.
