...
Class cards reserve 340px at ordinary desktop scales and share a compact target/microphone row, tuner and manual scoring. The target surface always pairs control/control-ink theme tokens, including material themes such as Vintage Audio. The microphone level uses an accessible meter and a fixed icon with on/off/paused states; it has no changing status text. Live pitch and signed cents use a fixed-height readout with a dash for unavailable evidence. Guidance, microphone errors, and scoring remain distinct from meter telemetry. Geometry tests exercise silence, noisy input, pitch changes, pause, saved ratings and turn changes under offline file URLs. No saved data or scoring rules change.
